...
class Home extends BaseController {
    ...
    public function validate_login(){
        ...
        if (!$this->validation->withRequest($this->request)->run()) {
            ...
            administracion,reportes,codigo_socio,miembros,usuarios.estado as estado,usuarios.created_at as miembro_desde')
                    ...
                    ->join('socios', 'socios.idusuario=usuarios.id','left')
                    ->join('rangos', 'rangos.id=socios.idrango','left')
                    ->join('roles', 'roles.id=usuarios.idrol','left')
                    ->findAll();
    ...
    public function contactos(){
        ...
        if ($data['logged'] == 1 ) {
            ...
            $data['title'] = NOMBRE_EMPRESA;
            $data['subtitle']='Contactos';
            $data['main_content'] = 'home/contactos';
            return view('dashboard/index', $data);
        }else{
            $this->logout();
        }
